...
$.pjax = function( options ) {
  ...
  var defaults = {
    ...
    success: function(data){
      ...
      // Or per https://github.com/defunkt/jquery-pjax use 
      // the data-title tag content if present.
      if(!title){
        tag = $.trim($container.find("[data-title]"))
        if(tag.html().length > 0) title = tag.html()
        if(tag.attr("data-title").length > 0) title = tag.attr("data-title")
      }
      
      // Updates the title, if any was found
